...
class Repository {
  ...
  async createNewObjectContent(id, writeContent, additive) {
    // Add an object to the repository with a callback that provides the content.
    // This is called by importNewObjectDor under the hood
    ...
    // id = an optional id String
    // additive = keep all existing content in the object as well as the new stuff
    ...
    if (!(await fs.pathExists(objectRepoPath))) {
      ...
      // Merge object
      const added = await this.mergeObjectWith(object, objectRepoPath, additive);
  ...
  async getObject(id) {
    const p = path.join(this.path, this.objectIdToPath(id));
    if (await fs.exists(p)) {
         const objToExport = new OcflObject();
          await objToExport.load(p);
          return objToExport;
    }
    else {
  ...
  async mergeObjectWith(newObject, prevObjectPath, additive) {
    ...
    if (Object.keys(newState).length === Object.keys(prevState).length) {
      ...
      for (let hash of Object.keys(newState)) {
        ...
        // FIXME: need to compare the two lists of paths, see issue 1411
      }
      if (same) {
        // First, do no harm
        // ie do nothing
    ...
    // If this is an 'additive' merge, bring everything from the previous state
    // into this state

    if( additive ) {
      const newh = Object.keys(newState);
      for ( let hash in prevState ) {
        for( let p of prevState[hash] ) {
          const nps = newh.filter((h) => newState[h].includes(p));
          if( nps.length > 0 ) {
            // this path is already in newState, so that version supersedes this
          } else {
            if( hash in newState ) {
              newState[hash].push(p)
            } else {
              newState[hash] = [ p ];
            }
          }
