...
const { flattenDeep, orderBy, groupBy, cloneDeep } = require("lodash");
...
class OcflObject {
  constructor({ id, path }) {
    ...
    if (id && !path) {
      this.path = pairtree.path(id);
      this.id = id;
    }
  }

  async writeInventories(inv, version) {
    const main_inv = await fs.writeJson(
      path.join(this.path, "inventory.json"),
      inv,
      { spaces: 2 }
    );
    const version_inv = await fs.writeJson(
      path.join(this.path, version, "inventory.json"),
      inv,
      { spaces: 2 }
    );
    const inv_hash = await this.hash_file(
      path.join(this.path, "inventory.json")
    );
    const digest_file_v1 = await fs.writeFile(
      path.join(this.path, version, "inventory.json." + DIGEST_ALGORITHM),
      inv_hash + "   inventory.json"
    );
    const digest_file = await fs.writeFile(
      path.join(this.path, "inventory.json." + DIGEST_ALGORITHM),
      inv_hash + "   inventory.json"
    );
  }
  ...
  async load() {
    ...
    if ((await fs.pathExists(this.path)) && stats.isDirectory()) {
      ...
      let inventory = path.join(this.path, "inventory.json");
      inventory = await fs.readJson(inventory);

      this.versions = await getVersionStates({
        inventory,
        objectRoot: this.path
      });
      // console.log(JSON.stringify(this.versions, null, 2));
    } else {
      throw new Error(path + " does not exist or is not a directory");
    }

    async function getVersionStates({ inventory, objectRoot }) {
      let states = [];
      ...
      for (let version of versions) {
        inventory = await fs.readJson(
          path.join(objectRoot, `v${version}`, "inventory.json")
        );
        let state = getVersionState({ inventory });
        states.push({
          version: `v${version}`,
          state
        });
    ...
    function getVersionState({ inventory }) {
      let files = [];
  ...
  getCurrentVersion() {
    return cloneDeep(this.versions).pop();
  }
