...
const { flattenDeep, orderBy, groupBy, cloneDeep, uniq } = require("lodash");
...
class OcflObject {
  constructor({ id, objectPath, ocflRoot }) {
    ...
    // if (objectPath) {
    //   this.path = path.join(ocflRoot, objectPath);
    // } else if (id && !objectPath) {
    //   this.id = id;
    //   // Need to rethink this relative to repositoryObject being passed in
    //   // const repo = new Repository({ ocflRoot });
    //   this.path = path.join(ocflRoot, pairtree.path(id));
    // }
    this.id = id;
    this.depositPath = path.join(ocflRoot, "deposit", pairtree.path(id));
    this.repositoryPath = path.join(ocflRoot, pairtree.path(id));
    this.backupPath = path.join(ocflRoot, "backup", pairtree.path(id));
  }
  ...
  async update({ source = undefined, writer = undefined }) {
    ...
    return new Promise(async resolve => {
      ...
      const { version, target } = await this.__initObject();

      if (writer) {
        await writer({ target });
      } else if (source) {
        // copy content to the new version folder
        await fs.copy(source, target);
      }

      // get last inventory
      let lastVersionInventory = await this.getLatestInventory();
      // console.log("last", lastVersionInventory);
      ...
      // console.log("current", currentVersionInventory);
  ...
  async __writeVersion({ inventory }) {
    ...
    try {
      ...
      // something went wrong - roll back original object
      await fs.move(this.backupPath, this.repositoryPath);
      // await fs.remove(this.depositPath);
    }
  }
  ...
  async __generateCurrentInventory({
    ...
    // map the old versions in to the current inventory
    currentVersionInventory.versions = {
      ...lastVersionInventory.versions,
      ...currentVersionInventory.versions
    };

    // return the currentVersionInventory
