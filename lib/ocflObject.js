...
const { flattenDeep, orderBy, groupBy } = require("lodash");
...
class OcflObject {
  ...
  async load() {
    ...
    if ((await fs.pathExists(this.path)) && stats.isDirectory()) {
      ...
      this.currentState = getCurrentState({ inventory });
      // console.log(JSON.stringify(this.currentState, null, 2));
      this.versions = await getVersionState({
        inventory,
    ...
    function getCurrentState({ inventory }) {
      let files = [];
    ...
    async function getVersionState({ inventory, objectRoot }) {
      let states = [];
      ...
      for (let version of versions) {
        ...
        let state = getCurrentState({ inventory });
        states.push({
          version: `v${version}`,
          state
        });
